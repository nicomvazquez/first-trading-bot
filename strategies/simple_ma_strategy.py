...
import pandas as pd
import config # Para acceder a los parámetros de la estrategia (SMA_SHORT_PERIOD, SMA_LONG_PERIOD)
...
def generate_signal(df_klines: pd.DataFrame) -> str:
    ...
    if df_klines.empty or len(df_klines) < config.SMA_LONG_PERIOD:
        print(f"DEBUG: No hay suficientes datos para la estrategia. Necesita {config.SMA_LONG_PERIOD} velas, tiene {len(df_klines)}.")
    ...
    if len(df_klines) < config.SMA_LONG_PERIOD:
        print(f"DEBUG: No hay suficientes datos válidos después de la limpieza. Necesita {config.SMA_LONG_PERIOD} velas, tiene {len(df_klines)}.")
        return "WAIT"

    # 3. Calcular las Medias Móviles Simples (SMA)
    df_klines['SMA_Short'] = df_klines['close'].rolling(window=config.SMA_SHORT_PERIOD).mean()
    df_klines['SMA_Long'] = df_klines['close'].rolling(window=config.SMA_LONG_PERIOD).mean()
    ...
    if last_row['SMA_Short'] > last_row['SMA_Long'] and \
       prev_row['SMA_Short'] <= prev_row['SMA_Long']: # El cruce ocurrió en la última vela
        print(f"DEBUG: Señal detectada: BUY (SMA Corta {last_row['SMA_Short']:.2f} > SMA Larga {last_row['SMA_Long']:.2f}, antes era <=)")
        return "BUY" 
    
    # Cruce bajista (Death Cross): SMA Corta cruza por debajo de SMA Larga
    elif last_row['SMA_Short'] < last_row['SMA_Long'] and \
         prev_row['SMA_Short'] >= prev_row['SMA_Long']: # El cruce ocurrió en la última vela
        print(f"DEBUG: Señal detectada: SELL (SMA Corta {last_row['SMA_Short']:.2f} < SMA Larga {last_row['SMA_Long']:.2f}, antes era >=)")
        return "SELL"
    
    # Si no hay cruce, no se toma ninguna acción
    else:
        print(f"DEBUG: Señal detectada: HOLD (No hay cruce claro. SMA Corta: {last_row['SMA_Short']:.2f}, SMA Larga: {last_row['SMA_Long']:.2f})")
        return "HOLD"